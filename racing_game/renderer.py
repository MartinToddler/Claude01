"""
Pygame rendering: track surface, cars, sensor rays, selection highlight.

New in v2:
  • hit_test(mx, my) — returns agent index under the mouse (or -1)
  • Selected car drawn with white outline + extra highlight ring
"""
...
class Renderer:
    def __init__(self, surface: pygame.Surface):
        ...
        self.selected_idx  = -1     # index into pop.agents, -1 = none
    ...
    def prerender_track(self, track: Track):
        ...
        pygame.draw.polygon(surf, C["track"], outer + inner[::-1])
    ...
    def hit_test(self, mx: int, my: int, pop: PopulationManager) -> int:
        """Return index of agent whose car bounding box contains (mx, my)."""
        from car import Car
        RADIUS = Car.LENGTH_PX + 4
    ...
    def _draw_agents(self, pop: PopulationManager):
        ...
        for i, agent in enumerate(pop.agents):
            ...
            color    = C["cars"][-1] if is_best else C["cars"][i % (len(C["cars"]) - 1)]
            ...
            if not car.alive:
                ...
                if self.show_rays and (is_best or is_sel):
                    self._draw_rays(car, pop.track if hasattr(pop, 'track') else None)
    ...
    def _car_corners(self, car):
        ...
        for lx, ly in [(hl, hw), (hl, -hw), (-hl, -hw), (-hl, hw)]:
            corners.append((
                int(car.x + lx * cos_h - ly * sin_h),
                int(car.y + lx * sin_h + ly * cos_h),
            ))
        return corners

    def _draw_car(self, car, color, is_best: bool, is_selected: bool):
        corners = self._car_corners(car)

        if is_selected:
            # Glow ring
            cx, cy = int(car.x), int(car.y)
            pygame.draw.circle(self.surface, (255, 255, 80), (cx, cy), 18, 2)
